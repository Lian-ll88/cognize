...
- **🔍 知识蒸馏 (Distill)**：利用 DeepSeek AI 将长文、笔记、网页等原始信息提炼为"一句话结论"、"关键判断"和"可复用表述"。
- **🔗 智能关联 (Relate)**：基于 **本地向量嵌入 (Local Embeddings)** 自动发现新旧知识间的关系，构建动态知识网络。
...
- **AI 模型**：
  - **DeepSeek Chat** (知识蒸馏与关系分析)
  - **Transformers.js (BGE-Small-ZH)** (本地向量化，无需 API Key)
...
> **注意**：首次运行蒸馏或搜索功能时，浏览器会自动下载约 30MB 的本地 Embedding 模型文件，请耐心等待。
